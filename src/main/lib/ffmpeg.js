...
const ffprobeBin =
  process.env.FFPROBE_PATH || ffprobeStatic.path.replace('app.asar', 'app.asar.unpacked')
...
async function trimClip(
  ...
  const args = ['-y', '-hide_banner', '-ss', start, '-i', inputPath, '-t', duration]
...
async function removeDeadSilence(
  inputClipPath,
  outputClipPath,
  silenceThreshold,
  silenceDuration,
...
async function mergeOutro(
  ...
  const filterComplex =
    `[1:v]scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2,setsar=1:1,setdar=${dar},fps=${fps},format=yuv420p[v1];` +
...
export { trimClip, removeDeadSilence, mergeOutro }
