    ...
    .map(async () => {
      while (rowIndex < validRows.length) {
        ...
        const showHeading =
          !disableHeading &&
          showHeadingVal &&
          String(showHeadingVal).trim().toLowerCase() === 'true'
        ...
        const step1Out =
          needsSilenceRemoval || needsOutro
            ? path.join(tempDir, `${String(id).padStart(2, '0')}_step1.mp4`)
            : outputPath

        sendProgress({ type: 'clip_progress', id, status: 'Processing', progress: 10 })
        await trimClip(inputVideo, step1Out, start, duration, headingOptions, aspectRatio, encoder)
        sendProgress({
          type: 'clip_progress',
          id,
          status: 'Processing',
          progress: needsSilenceRemoval ? 40 : needsOutro ? 60 : 100
        })
        ...
        if (needsSilenceRemoval) {
          ...
          sendProgress({
            ...
            id,
            status: 'Processing',
            progress: needsOutro ? 75 : 100
          })

          try {
            fs.unlinkSync(lastOut)
          } catch {
            /* ignore */
          }
          lastOut = step2Out
        ...
        if (needsOutro) {
          ...
          await mergeOutro(lastOut, outroVideo, outputPath, aspectRatio, encoder)
